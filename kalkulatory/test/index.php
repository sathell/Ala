...
<head>
    ...
    <meta name="description" content="Podaj kwotę, okres i oprocentowanie kredytu — kalkulator sam wyliczy aktualne saldo i pokaże, ile zaoszczędzisz dzięki regularnym, stałym nadpłatom miesięcznym.">
    ...
    <meta property="og:description" content="Oblicz efekt regularnych, comiesięcznych nadpłat kredytu hipotecznego — bez podawania aktualnego salda.">
    ...
    <style>
        :root {
            --ink: #16241c;
            --accent: #24402f;      /* leśna zieleń — akcent główny, zamiast płaskiej czerni */
            --accent-soft: #eaf0ea;
            --brass: #a8763e;       /* stonowany mosiądz — akcent drugorzędny / linia "bez nadpłat" */
            --line: rgba(22,36,28,0.09);
            --line-soft: rgba(22,36,28,0.05);
            --paper: #ffffff;
            --paper-tint: rgba(22,36,28,0.025);
        }

        /* --- komponenty przejęte 1:1 z kalkulatora nadpłaty jednorazowej --- */
        .kalk-alert { grid-column: 1 / -1; margin-top: 6px; padding: 14px 18px; border-radius: 8px; border-left: 4px solid; font-family: 'Jost', sans-serif; font-size: 0.95rem; line-height: 1.55; }
        .kalk-alert strong { display: block; margin-bottom: 4px; font-family: 'Cormorant Garamond', serif; font-size: 1.15rem; font-weight: 500; }
        .kalk-alert--error { background: #fdecec; border-left-color: #c0392b; color: #7a1f17; }
        .kalk-alert--warning { background: #fff5e6; border-left-color: #e08a1e; color: #6e4a0e; }
        .kalk-btn:disabled { opacity: 0.5; cursor: not-allowed; }
        .kalk-teaser { margin-top: 28px; padding: 22px 24px; border: 1px solid var(--line); border-radius: 10px; background: var(--paper-tint); }
        .kalk-teaser-label { font-family: 'Jost', sans-serif; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.12em; opacity: 0.6; margin-bottom: 8px; }
        .kalk-teaser-title { font-family: 'Cormorant Garamond', serif; font-weight: 500; font-size: 1.35rem; line-height: 1.3; margin: 0 0 10px; }
        .kalk-teaser-text { margin: 0 0 14px; line-height: 1.65; }
        .kalk-teaser-link { font-family: 'Jost', sans-serif; font-weight: 500; text-decoration: underline; text-underline-offset: 3px; }

        /* --- nowe komponenty potrzebne dla kalkulatora stałych nadpłat --- */
        .kalk-pillrow { display: flex; flex-wrap: wrap; gap: 8px; margin: 8px 0 4px; }
        .kalk-pill {
            font-family: 'Jost', sans-serif; font-size: 0.85rem; padding: 7px 16px; border-radius: 999px;
            border: 1px solid rgba(22,36,28,0.16); background: #fff; cursor: pointer; transition: all .15s ease; user-select: none;
        }
        .kalk-pill:hover { border-color: var(--accent); }
        .kalk-pill.active { background: var(--accent); border-color: var(--accent); color: #fff; }

        .kalk-substep-label { font-family: 'Jost', sans-serif; font-size: 0.78rem; text-transform: uppercase; letter-spacing: 0.08em; opacity: 0.55; margin: 16px 0 2px; }

        .kalk-slider-wrap { margin-top: 4px; }
        .kalk-slider-wrap input[type=range] { width: 100%; accent-color: var(--accent); }
        .kalk-slider-read { font-family: 'Jost', sans-serif; font-size: 0.85rem; text-align: right; opacity: 0.7; margin-top: 2px; }

        .kalk-ministats { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; margin-top: 14px; }
        .kalk-ministat {
            background: var(--paper); border: 1px solid var(--line-soft); border-radius: 8px; padding: 12px 14px;
            box-shadow: 0 1px 2px rgba(22,36,28,0.03);
        }
        .kalk-ministat-label { font-family: 'Jost', sans-serif; font-size: 0.66rem; text-transform: uppercase; letter-spacing: 0.06em; opacity: 0.55; }
        .kalk-ministat-val { font-family: 'Cormorant Garamond', serif; font-size: 1.4rem; font-weight: 500; color: var(--ink); margin-top: 1px; }

        /* Taśma sumująca — motyw "adding machine tape": perforowana krawędź u góry */
        .kalk-tape {
            position: relative; margin-top: 24px; padding: 24px 26px 22px; border: 1px solid var(--line); border-radius: 10px;
            background:
                radial-gradient(circle, var(--paper) 5px, transparent 5.5px) top / 22px 14px repeat-x,
                repeating-linear-gradient(45deg, #fff, #fff 11px, var(--paper-tint) 11px, var(--paper-tint) 22px);
            box-shadow: 0 1px 2px rgba(22,36,28,0.04), 0 10px 28px -18px rgba(22,36,28,0.35);
            display: flex; align-items: center; justify-content: space-between; gap: 16px; flex-wrap: wrap;
        }
        .kalk-tape-label { font-family: 'Jost', sans-serif; font-size: 0.74rem; text-transform: uppercase; letter-spacing: 0.12em; opacity: 0.55; }
        .kalk-tape-val { font-family: 'Cormorant Garamond', serif; font-size: 2rem; font-weight: 500; color: var(--accent); margin-top: 3px; }
        .kalk-tape-side { font-family: 'Jost', sans-serif; font-size: 0.92rem; text-align: right; color: var(--accent); }
        .kalk-tape-side span { opacity: 0.6; color: var(--ink); }

        .kalk-chart-block { margin-top: 32px; }
        .kalk-chart-title { font-family: 'Cormorant Garamond', serif; font-size: 1.3rem; font-weight: 500; margin-bottom: 2px; color: var(--ink); }
        .kalk-chart-sub { font-family: 'Jost', sans-serif; font-size: 0.8rem; opacity: 0.55; margin-bottom: 10px; }
        .kalk-chart-canvas-wrap {
            border: 1px solid var(--line-soft); border-radius: 10px; padding: 14px 12px 8px; background: var(--paper);
            box-shadow: 0 1px 2px rgba(22,36,28,0.03), 0 8px 22px -18px rgba(22,36,28,0.4);
        }
        .kalk-chart-legend { display: flex; gap: 18px; margin-top: 10px; font-family: 'Jost', sans-serif; font-size: 0.8rem; }
        .kalk-chart-legend span { display: inline-flex; align-items: center; gap: 6px; }
        .kalk-chart-legend i { width: 12px; height: 12px; border-radius: 2px; display: inline-block; }

        .kalk-scenario-table, .kalk-expert-table, .kalk-schedule-table { width: 100%; border-collapse: collapse; margin-top: 10px; font-family: 'Jost', sans-serif; font-size: 0.88rem; }
        .kalk-scenario-table th, .kalk-expert-table th, .kalk-schedule-table th {
            text-align: right; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.05em; opacity: 0.55; font-weight: 500;
            padding: 8px 10px; border-bottom: 1px solid var(--line); white-space: nowrap;
        }
        .kalk-scenario-table th:first-child, .kalk-expert-table th:first-child, .kalk-schedule-table th:first-child,
        .kalk-scenario-table td:first-child, .kalk-expert-table td:first-child, .kalk-schedule-table td:first-child { text-align: left; }
        .kalk-scenario-table td, .kalk-expert-table td, .kalk-schedule-table td { text-align: right; padding: 8px 10px; border-bottom: 1px solid var(--line-soft); white-space: nowrap; font-variant-numeric: tabular-nums; }
        .kalk-scenario-table tbody tr:hover td, .kalk-expert-table tbody tr:hover td, .kalk-schedule-table tbody tr:hover td { background: var(--paper-tint); }
        .kalk-expert-table tr.is-active td { background: var(--accent-soft); font-weight: 600; color: var(--accent); }
        ...
        .kalk-table-wrap { overflow-x: auto; -webkit-overflow-scrolling: touch; border-radius: 10px; }
        .kalk-schedule-table { min-width: 620px; }
        .kalk-expert-table { min-width: 440px; }
        .kalk-scenario-table { min-width: 380px; }
        @media (max-width: 480px) {
            .kalk-scenario-table, .kalk-expert-table, .kalk-schedule-table { font-size: 0.82rem; }
            .kalk-scenario-table th, .kalk-expert-table th, .kalk-schedule-table th,
            .kalk-scenario-table td, .kalk-expert-table td, .kalk-schedule-table td { padding: 6px 8px; }
        }

        .kalk-toggle-link { display: inline-block; margin-top: 10px; font-family: 'Jost', sans-serif; font-size: 0.85rem; text-decoration: underline; text-underline-offset: 3px; cursor: pointer; color: var(--accent); }

        /* Karty wyników i rekomendacja — lekka głębia zamiast płaskich paneli */
        .kalk-metric, .kalk-rec { box-shadow: 0 1px 2px rgba(22,36,28,0.03), 0 10px 26px -20px rgba(22,36,28,0.4); }
        .kalk-metric--accent .kalk-metric-val { color: var(--accent); }
    </style>
    <!-- Uwaga: zmienne --accent / --brass sterują akcentami kalkulatora; jeśli marka ma
         już zdefiniowany kolor przewodni w main.css, podmień tu dwie wartości wyżej. -->
        ...
        <div class="page">
            ...
            <main class="content">
                ...
                <div class="page-header">
                    ...
                    <p class="page-lead">Nie musisz znać aktualnego salda. Podaj kwotę kredytu, okres, oprocentowanie i miesiąc uruchomienia — resztę, łącznie z dzisiejszym saldem, policzy kalkulator na podstawie pełnego harmonogramu spłat.</p>
                    <div class="rule"></div>
                </div>
                        ...
                        <div class="kalk-field">
                            ...
                            <div class="kalk-input-wrap" style="gap:8px;">
                                <select id="miesiac-start">
                                    ...
                                    <option value="6" selected>lipiec</option><option value="7">sierpień</option><option value="8">wrzesień</option>
                                    <option value="9">październik</option><option value="10">listopad</option><option value="11">grudzień</option>
                                </select>
                                <input type="number" id="rok-start" value="2023" min="1990" max="2035" step="1" style="max-width:90px;">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="kalk-block">
                    ...
                    <p class="kalk-hint" style="margin-top:-6px;">Podpowiedź liczona względem dzisiejszej daty — możesz ją skorygować suwakiem.</p>
                ...
                <div class="kalk-block">
                    ...
                    <button class="kalk-btn" id="oblicz-btn" onclick="oblicz(true)" style="margin-top:18px;">Oblicz efekt nadpłaty</button>
                </div>
                ...
                <div class="kalk-results visible" id="wyniki">
                    ...
                    <div class="kalk-chart-block">
                        ...
                        <p id="gdyby-tekst" style="font-family:'Jost',sans-serif; font-size:0.92rem; line-height:1.6;">—</p>
                        <div class="kalk-table-wrap">
                        <table class="kalk-scenario-table">
                            <thead><tr><th>Rozpoczęcie nadpłat</th><th>Oszczędność odsetkowa</th></tr></thead>
                    ...
                    <div class="kalk-chart-block">
                        ...
                        <div class="kalk-chart-legend">
                            <span><i style="background:#a8763e;"></i>bez nadpłat</span>
                            <span><i style="background:#24402f;"></i>z nadpłatami</span>
                        </div>
                    </div>
                    ...
                    <div class="kalk-chart-block">
                        ...
                        <div class="kalk-chart-canvas-wrap"><canvas id="chart-struktura" height="220"></canvas></div>
                        <div class="kalk-chart-legend">
                            <span><i style="background:#24402f;"></i>kapitał + nadpłata</span>
                            <span><i style="background:#a8763e;"></i>odsetki</span>
                        </div>
                    </div>
                    ...
                    <div class="kalk-chart-block">
                        ...
                        <table class="kalk-expert-table">
                            <thead><tr><th>Nadpłata</th><th>Koniec kredytu</th><th>Odsetki łącznie</th></tr></thead>
                    ...
                    <div class="kalk-chart-block">
                        ...
                        <div class="kalk-table-wrap">
                        <table class="kalk-expert-table">
                            <thead><tr><th>Wariant</th><th>Dodatkowe miesiące</th><th>Dodatkowa oszczędność</th></tr></thead>
                    ...
                    <div class="kalk-chart-block">
                        ...
                        <table class="kalk-schedule-table">
                            <thead><tr><th>Rata</th><th>Saldo</th><th>Kapitał</th><th>Odsetki</th><th>Nadpłata</th><th>Nowe saldo</th></tr></thead>
                            <tbody id="schedule-body"></tbody>
                        </table>
                        </div>
                        <span class="kalk-toggle-link" id="schedule-toggle" onclick="toggleSchedule()">Pokaż pełny harmonogram</span>
                    </div>
                ...
                <div class="kalk-disclaimer">
                    <p><strong>Uwaga:</strong> Kalkulator zakłada stałe oprocentowanie przez cały okres kredytowania i brak wcześniejszych nadpłat lub zmian oprocentowania — na tej podstawie szacuje aktualne saldo. Wyniki mają charakter informacyjny i poglądowy. Rzeczywiste warunki (w tym ewentualna prowizja za nadpłatę w pierwszych 3 latach umowy) zależą od Twojego banku. Skontaktuj się ze swoim bankiem lub doradcą kredytowym przed podjęciem decyzji.</p>
                </div>

                <div class="kalk-teaser">
                    <div class="kalk-teaser-label">Komentarz eksperta</div>
                    <div class="kalk-teaser-title">Regularna nadpłata kontra jednorazowa wpłata — co daje większą oszczędność?</div>
                    <p class="kalk-teaser-text">Jednorazowa wpłata robi wrażenie, ale to regularność i moment startu nadpłat mają zwykle większy wpływ na całkowity koszt kredytu. Sprawdź, jak niewielka, systematyczna kwota miesięcznie zmienia harmonogram na przestrzeni lat.</p>
                    <a href="/komentarze/nadplata-kredytu-hipotecznego/" class="kalk-teaser-link">Czytaj cały komentarz →</a>
                </div>
...
function rysujWykresLiniowy(canvasId, serie, kolory, formatX) {
    ...
    // siatka — subtelne przerywane linie zamiast pełnych, spokojniejszy rytm
    ctx.strokeStyle = 'rgba(22,36,28,0.07)'; ctx.lineWidth = 1; ctx.setLineDash([3, 4]);
    for (let i = 0; i <= 4; i++) {
        const y = padT + h - (h * i / 4);
        ctx.beginPath(); ctx.moveTo(padL, y); ctx.lineTo(padL + w, y); ctx.stroke();
        ctx.fillStyle = 'rgba(22,36,28,0.5)'; ctx.font = '11px Jost, sans-serif'; ctx.textAlign = 'right';
        ctx.fillText(Math.round(maxY * i / 4 / 1000) + 'k', padL - 8, y + 4);
    }
    ctx.setLineDash([]);

    serie.forEach(function (s, idx) {
        ctx.strokeStyle = kolory[idx]; ctx.lineWidth = 2.25; ctx.lineJoin = 'round'; ctx.lineCap = 'round';
        ctx.beginPath();
        const pts = s.data.map(function (p) {
            return { x: padL + (p.x / maxX) * w, y: padT + h - (p.y / maxY) * h };
        });
        // wygładzenie linii krzywymi przez środki odcinków, zamiast łamanych prostych
        if (pts.length) {
    ...
    ctx.fillStyle = 'rgba(22,36,28,0.5)'; ctx.font = '11px Jost, sans-serif'; ctx.textAlign = 'center';
    for (let i = 0; i <= 4; i++) {
        const x = padL + (w * i / 4);
...
function rysujWykresStruktury(canvasId, dane) {
    ...
    dane.forEach(function (d, i) {
        ...
        const gap = barW > 3 ? 1.5 : 0;
        ctx.fillStyle = '#a8763e';
        ctx.fillRect(x, y0, Math.max(1, barW - gap), totalH - kapH);
        ctx.fillStyle = '#24402f';
        ctx.fillRect(x, y0 + (totalH - kapH), Math.max(1, barW - gap), kapH);
    });
...
function oblicz(przewin) {
    ...
    rysujWykresLiniowy('chart-saldo', [{ data: seriaSaldoBez }, { data: seriaSaldoZ }], ['#a8763e', '#24402f']);
    rysujWykresLiniowy('chart-odsetki', [{ data: seriaOdsetkiBez }, { data: seriaOdsetkiZ }], ['#a8763e', '#24402f']);
